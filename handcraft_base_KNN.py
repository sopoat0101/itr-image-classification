...
def handCraft():
    ...
    knnModel = KNeighborsClassifier(n_neighbors=12).fit(features, labels)
    #   Save model file.
    pickle.dump(knnModel, open(MODEL_PATH + '/hancraft_model_KNN.sav', 'wb'))
    #   Save features and labels file.
    pickle.dump([features, labels], open(
        MODEL_PATH + '/KNN_' + HAND_CRAFT_OUTPUT_FILE_NAME, 'wb'))
    print('Learn Done!')
